...
namespace mango::ct {
    ...
    struct Nat {
        ...
        constexpr const Nat<>high() const noexcept {
            return {};
        }
        ...
        constexpr const Nat<1> succ() const noexcept;

        template <uint64_t ...Rs>
        constexpr Cmp cmp(const Nat<Rs...> &rhs, const Cmp prev = Cmp::EQ) const noexcept {
            if (rhs.is_zero()) {
                return prev;
            } else {
                return Cmp::LT;
    ...
    struct Nat<Head, Tail...>  {

        constexpr static uint64_t low = Head;

        constexpr const Nat<Tail...> high() const noexcept {
            return {};
        }
        ...
        template <uint64_t ...Rs>
        constexpr const auto add_with_carry(const Nat<Rs...> &rhs, const bool carry_in = false) const noexcept {
            const auto new_low = low + rhs.low + carry_in ? 1 : 0;
            const auto new_carry = new_low < low || (new_low < rhs.low);

            return high().add_with_carry(rhs.high(), new_carry).template inject_right<new_low>();  
        }
    ...
    template <uint64_t... Vs>
    constexpr const Nat<1> Nat<Vs...>::succ() const noexcept {
        return {};
    }
